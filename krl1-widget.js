...
(function () {
  ...
  styleEl.textContent = [
    ...
    /* Liquid edge — rotating conic-gradient ring (animates only when open) */
    ...
    /* Top inner specular line — the signature glass highlight */
    ...
    /* ── INPUT ROW : glass pills + liquid focus halo ─────────────────────── */
    ...
    /* ── SEND BUTTON : specular sweep on hover ───────────────────────────── */
  ...
  var TOOL_CONTEXT = {
    ...
    'product-brief': {
      fr: "Tu es sur le **Product Brief** 📄 C'est la synthèse exportable de ta session : discovery, recherche, OKRs, priorisation, user stories et roadmap réunis en un document (PDF, Markdown, ou export one-click vers **Notion** en OAuth). Je peux t'expliquer comment c'est généré ou comment l'exploiter face à des stakeholders. Qu'est-ce qui t'intéresse ?",
      en: "You're on the **Product Brief** 📄 It's the exportable synthesis of your session: discovery, research, OKRs, prioritisation, user stories and roadmap in one document (PDF, Markdown, or one-click export to **Notion** via OAuth). I can explain how it's generated or how to use it with stakeholders. What are you curious about?",
      chips_fr: ["📄 Comment est généré le brief ?", "📤 Exporter vers Notion, PDF ou Markdown ?", "🗂️ Gérer plusieurs projets ?"],
      chips_en: ["📄 How is the brief generated?", "📤 Export to Notion, PDF or Markdown?", "🗂️ Manage multiple projects?"]
    },
    'pm-toolkit': {
      fr: "Tu es sur le **PM Toolkit** 🧰 6 outils enchaînés, du problème utilisateur jusqu'à la roadmap, avec un Product Brief exportable au bout (PDF, Markdown, ou vers **Notion** en un clic). Je peux t'aider à choisir par où commencer ou t'expliquer comment les outils se transmettent le contexte. Qu'est-ce qui t'amène ?",
      en: "You're on the **PM Toolkit** 🧰 6 chained tools, from user problem to roadmap, with an exportable Product Brief at the end (PDF, Markdown, or to **Notion** in one click). I can help you pick where to start or explain how the tools pass context to each other. What brings you here?",
  ...
  var SYSTEM_PROMPT =
    ...
    "- Cloudflare Worker 'groq-proxy' (JS), routes : POST / (proxy Groq direct), /orchestrate (planner+synthesis sync), /orchestrate-stream (synthesis SSE), /rag-query (RAG sémantique), /feedback (analyse feedback), GET /stats (compteur KV), GET|POST /veille\n" +
    "- Flow /orchestrate-stream : Appel #1 Planner (llama-3.3-70b, temp 0.2, JSON strict : intent/confidence/user_goal/steps) → Appel #2 Synthesis (temp 0.45, SSE stream). ctx.waitUntil pour Langfuse + KV non-bloquant\n" +
    "- RAG (/rag-query) : Workers AI bge-small-en-v1.5 (embeddings Cloudflare native), cosine similarity inline dans le Worker, KB de 15 documents PM/Agile/AI Product en code (chunks 300 mots, overlap 50), top-3 chunks injectés si intent=pm_workflow\n" +
    "- Observabilité : Langfuse Cloud free tier (50k events/mois), traces span planner + span retriever + span synthesis sur les 3 routes IA, flush non-bloquant via ctx.waitUntil\n" +
    "- Routing widget : KB fast-path local (0 appel API) → PM_REGEX → /orchestrate-stream → fallback /orchestrate\n" +
    "- Secrets Worker : GROQ_KEY, LANGFUSE_PUBLIC_KEY, LANGFUSE_SECRET_KEY, MAKE_WEBHOOK_URL, MAKE_SECRET. Déploiement : npx wrangler deploy --config wrangler.toml\n" +
  ...
  var KB = {
    fr: { rules: [
      ...
      { k: ["rag","retrieval","embedding","bge","workers ai","base de connaissance","chunks","cosine","sémantique","similarité"],
        r: "La pipeline <strong>RAG</strong> du site (route /rag-query) :\n\n• <strong>Embedding</strong> : Workers AI bge-small-en-v1.5 (Cloudflare native, $0)\n• <strong>KB</strong> : 15 documents PM/Agile/AI Product encodés en chunks de 300 mots (overlap 50 mots)\n• <strong>Recherche</strong> : cosine similarity calculée inline dans le Worker\n• <strong>Activation</strong> : uniquement si intent = pm_workflow\n• <strong>Top-K</strong> : 3 chunks les plus proches injectés dans le prompt de synthesis\n\nPas de Pinecone, pas de Qdrant. Tout tourne dans un seul Worker isolate. La query + tous les chunks sont envoyés en un seul appel à Workers AI.",
        chips: ["🤖 Comment fonctionne KRL1 ?", "🏗️ Pourquoi Cloudflare Workers ?"] },
      { k: ["langfuse","observabilit","traces","monitoring","logs","télémétrie","instrumenté","langsmith","tracing"],
        r: "<strong>Langfuse Cloud</strong> (free tier 50k events/mois) instrumente les 3 routes IA :\n\n• <strong>/orchestrate</strong> : trace planner + synthesis\n• <strong>/orchestrate-stream</strong> : trace planner + synthesis streaming\n• <strong>/rag-query</strong> : trace planner + retriever + synthesis\n\nChaque trace capture : tokens input/output, latence par span, intent détecté, confidence, sources RAG. Flush via ctx.waitUntil (non-bloquant : la réponse n'attend pas Langfuse). Aucun SDK Langfuse, juste un fetch manuel vers l'API d'ingestion Langfuse.",
        chips: ["🏗️ Pourquoi Cloudflare Workers ?", "🤖 Comment fonctionne KRL1 ?"] },
      { k: ["stream","sse","server-sent","temps réel","progressif","delta","token par token","comment tu envoies"],
        r: "Le streaming dans KRL1 fonctionne via <strong>Server-Sent Events (SSE)</strong> :\n\n• Le Worker appelle Groq avec stream:true et retransmet le ReadableStream directement au navigateur\n• Le widget lit le stream chunk par chunk via getReader()\n• Chaque token JSON est parsé : choices[0].delta.content\n• Rendu progressif mot par mot (délai 22ms par mot) avec curseur ▋ animé\n• Le planner (appel #1) est toujours bloquant. Seul le synthesis (appel #2) streame\n• Fallback automatique vers /orchestrate (non-streaming) si le stream échoue",
        chips: ["🤖 Comment fonctionne KRL1 ?", "🏗️ Pourquoi Cloudflare Workers ?"] },
      { k: ["orchestr","planner","synthesis","deux appels","2 appels","pipeline","comment tu décides","intent","json strict"],
        r: "Le flow d'orchestration (<strong>/orchestrate-stream</strong>) en détail :\n\n1. <strong>Planner</strong> : Groq llama-3.3-70b, temp 0.2, response_format JSON → intent (pm_workflow / portfolio / tech / contact / other), confidence, user_goal, steps, risks, quick_win\n2. <strong>Synthesis</strong> : Groq llama-3.3-70b, temp 0.45, SSE stream → réponse finale adaptée à l'intent avec liens PM si besoin\n3. <strong>Background</strong> : ctx.waitUntil → flush Langfuse + incrément KV\n\nRouting widget : KB fast-path (0 API) → PM_REGEX → /orchestrate-stream → fallback /orchestrate",
        chips: ["🔍 Comment fonctionne le RAG ?", "📊 Langfuse, c'est quoi ?"] },
      { k: ["groq","llama","lpu","temperature","max_token","latence","quel modèle","llm","quelle vitesse","llama-3"],
        r: "Le LLM utilisé est <strong>llama-3.3-70b-versatile</strong> via l'API Groq :\n\n• <strong>Hardware</strong> : LPU (Language Processing Unit) Groq → 1-2s de latence totale\n• <strong>Planner</strong> : temp 0.2, max_tokens 500, JSON strict\n• <strong>Synthesis</strong> : temp 0.45, max_tokens 450, texte libre\n• <strong>Free tier Groq</strong> : 30 req/min, 14 400 tokens/min\n• Budget total LLM : $0\n\nPourquoi Groq : la vitesse LPU est critique pour le streaming — les tokens arrivent assez vite pour que l'UX soit fluide.",
        chips: ["🏗️ Pourquoi Cloudflare Workers ?", "🤖 Comment fonctionne KRL1 ?"] },
    ...
    en: { rules: [
      ...
      { k: ["rag","retrieval","embedding","bge","workers ai","knowledge base","chunks","cosine","semantic search"],
        r: "KRL1's <strong>RAG pipeline</strong> (route /rag-query):\n\n• <strong>Embedding</strong>: Workers AI bge-small-en-v1.5 (Cloudflare native, $0)\n• <strong>KB</strong>: 15 PM/Agile/AI Product documents encoded in 300-word chunks (50-word overlap)\n• <strong>Search</strong>: cosine similarity computed inline in the Worker\n• <strong>Activation</strong>: only when intent = pm_workflow\n• <strong>Top-K</strong>: 3 closest chunks injected into the synthesis prompt\n\nNo Pinecone, no Qdrant. Everything runs in a single Worker isolate. The query + all chunks are sent in one call to Workers AI.",
        chips: ["🤖 How does KRL1 work?", "🏗️ Why Cloudflare Workers?"] },
      { k: ["langfuse","observability","traces","monitoring","logs","telemetry","instrumented","tracing","langsmith"],
        r: "<strong>Langfuse Cloud</strong> (free tier, 50k events/month) instruments the 3 AI routes:\n\n• <strong>/orchestrate</strong>: trace planner + synthesis\n• <strong>/orchestrate-stream</strong>: trace planner + streaming synthesis\n• <strong>/rag-query</strong>: trace planner + retriever + synthesis\n\nEach trace captures: input/output tokens, latency per span, detected intent, confidence, RAG sources. Flushed via ctx.waitUntil (non-blocking — response doesn't wait for Langfuse). No Langfuse SDK: manual fetch to the ingestion API.",
      ...
      { k: ["stream","sse","server-sent","real-time","real time","progressive render","delta","token by token","how do you send"],
        r: "KRL1's streaming runs on <strong>Server-Sent Events (SSE)</strong>:\n\n• The Worker calls Groq with stream:true and pipes the ReadableStream directly to the browser\n• The widget reads the stream chunk by chunk via getReader()\n• Each JSON token is parsed: choices[0].delta.content\n• Progressive word-by-word render (22ms delay per word) with animated ▋ cursor\n• The planner (call #1) is always blocking. Only synthesis (call #2) streams\n• Auto-fallback to /orchestrate (non-streaming) if the stream fails",
        chips: ["🤖 How does KRL1 work?", "🏗️ Why Cloudflare Workers?"] },
      { k: ["orchestrat","planner","synthesis","two calls","2 calls","pipeline","how do you decide","intent","json format"],
        r: "The orchestration flow (<strong>/orchestrate-stream</strong>) in detail:\n\n1. <strong>Planner</strong>: Groq llama-3.3-70b, temp 0.2, response_format JSON → intent (pm_workflow / portfolio / tech / contact / other), confidence, user_goal, steps, risks, quick_win\n2. <strong>Synthesis</strong>: Groq llama-3.3-70b, temp 0.45, SSE stream → final response adapted to intent\n3. <strong>Background</strong>: ctx.waitUntil → Langfuse flush + KV counter increment\n\nWidget routing: KB fast-path (0 API) → PM_REGEX → /orchestrate-stream → fallback /orchestrate",
        chips: ["🔍 How does the RAG work?", "📊 What is Langfuse?"] },
      { k: ["groq","llama","lpu","temperature","max_token","latency","which model","llm","how fast","llama-3"],
        r: "The LLM is <strong>llama-3.3-70b-versatile</strong> via the Groq API:\n\n• <strong>Hardware</strong>: LPU (Language Processing Unit) → 1-2s total latency\n• <strong>Planner</strong>: temp 0.2, max_tokens 500, strict JSON\n• <strong>Synthesis</strong>: temp 0.45, max_tokens 450, free text\n• <strong>Groq free tier</strong>: 30 req/min, 14,400 tokens/min\n• Total LLM budget: $0\n\nWhy Groq: LPU speed is critical for streaming — tokens arrive fast enough to keep the UX fluid.",
        chips: ["🏗️ Why Cloudflare Workers?", "🤖 How does KRL1 work?"] },
